...
from src.pipelines.voice_pipeline import get_voice_embedding, VoiceEncoder, load_voice_encoder
...
def student_dashboard():
    ...
    with c2:
        if st.button('Enrolled in Subjects'):
            enroll_dialog()
    ...
    for i, sub_node in enumerate(subjects):
        ...
        stats = stats_map.get(sid,{"total": 0,"attendend": 0})
        def unenroll_btn():
...
def student_screen():
    ...
    if photo_source:
        ...
        with st.spinner('AI is scanning..'):
            deteected, all_ids, num_faces = predict_attendence(img)
            ...
            if num_faces == 0:
                ...
                if deteected:
                    student_id = list(deteected.keys())[0]
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name", placeholder='E.g. Manish')
            ...
            try:
                audio_data = st.audio_input('Record a short phrase like I m Present, My name is Manish ')
            except Exception:
                ...
                if new_name:
                    with st.spinner('Creating profile..'):
                        ...
                        if encodings:
                            ...
                            if response_data:
                                train_classifier()
                                st.session_state.is_logges_in = True
                                st.session_state.user_role = 'student'
